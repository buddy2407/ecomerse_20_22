...
def Add_To_Cart(request):
    ...
    if 'cartdata' in request.session:
        if str(request.GET['id']) in request.session['cartdata']:
            ...
            cartdata[str(request.session['id'])]['qty']=int(cart_p[str(request.GET['id'])]['qty'])
            cartdata.update(cartdata)
            request.session['cartdata']=cartdata
        else:
            carddata=request.session['cartdata']
            carddata.update(cart_p)
            request.session['cartdata']=carddata
    else:
